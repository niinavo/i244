...
function connect_db(){
	...
	or die("ei saa ühendust mootoriga- ".mysqli_error());
	mysqli_query($connection, "SET CHARACTER SET UTF8") 
	or die("Ei saanud baasi utf-8-sse - ".mysqli_error($connection));
}
...
function kuva_puurid(){
	...
	if(empty($_SESSION["user"])){
		...
		from 10153400_loomaaed order by puur asc");
		$puurid=array();
		while ($r=mysqli_fetch_assoc($p)){
			$l=mysqli_query($connection, "SELECT * FROM 10153400_loomaaed 
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			if($_POST["pass"] == '' || $_POST["user"] == ''){
				...
				if(empty($_POST["pass"]))
					...
					$sql = "SELECT id FROM 10153400_kylastajad 
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			if($_POST["puur"] == '' || $_POST["nimi"] == '' ){
				...
				if(empty($_POST["puur"])){
					...
					$sql = "INSERT INTO 10153400_loomaaed (nimi, puur, liik) 
